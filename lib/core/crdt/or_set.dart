...
class ORSet<T> {
  final Map<T, Set<String>> _elements; // element -> set of unique tags that added it
  ...
  ORSet._(this._nodeId, this._elements, this._removedTags, this._vectorClock, this._tagCounter);
  ...
  ORSet<T> add(T element) {
    ...
    final tag = '${_nodeId}_${_tagCounter}_${DateTime.now().millisecondsSinceEpoch}';
  ...
  ORSet<T> mergeWith(ORSet<T> other) {
    ...
    final mergedTagCounter = _tagCounter > other._tagCounter ? _tagCounter : other._tagCounter;
  ...
  int get hashCode {
    return Object.hash(
      _nodeId,
      _elements.entries.fold<int>(0, (hash, entry) => hash ^ Object.hash(entry.key, entry.value.fold<int>(0, (h, tag) => h ^ tag.hashCode))),
