...
class PNCounter {
  ...
  PNCounter._(this._nodeId, this._positiveCounters, this._negativeCounters,
      this._vectorClock);
  ...
  factory PNCounter.fromJson(Map<String, dynamic> json) {
    ...
    final positiveCounters =
        Map<String, int>.from(json['positive_counters'] as Map);
    final negativeCounters =
        Map<String, int>.from(json['negative_counters'] as Map);
  ...
  int get value {
    final positiveSum =
        _positiveCounters.values.fold<int>(0, (sum, count) => sum + count);
    final negativeSum =
        _negativeCounters.values.fold<int>(0, (sum, count) => sum + count);
  ...
  PNCounter increment([int amount = 1]) {
    ...
    return PNCounter._(_nodeId, newPositiveCounters,
        Map<String, int>.from(_negativeCounters), newVectorClock);
  }
  ...
  PNCounter decrement([int amount = 1]) {
    ...
    return PNCounter._(_nodeId, Map<String, int>.from(_positiveCounters),
        newNegativeCounters, newVectorClock);
  }
  ...
  PNCounter mergeWith(PNCounter other) {
    ...
    final allPositiveNodes = <String>{
      ..._positiveCounters.keys,
      ...other._positiveCounters.keys
    };
    ...
    final allNegativeNodes = <String>{
      ..._negativeCounters.keys,
      ...other._negativeCounters.keys
    };
    ...
    return PNCounter._(
        _nodeId, mergedPositive, mergedNegative, mergedVectorClock);
  }
  ...
  int get hashCode {
    return Object.hash(
      _nodeId,
      _positiveCounters.entries.fold<int>(
          0, (hash, entry) => hash ^ Object.hash(entry.key, entry.value)),
      _negativeCounters.entries.fold<int>(
          0, (hash, entry) => hash ^ Object.hash(entry.key, entry.value)),
