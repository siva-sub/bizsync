...
class HybridLogicalClock {
  ...
  HybridLogicalClock.fromTimestamp(
      int physicalTime, int logicalTime, String nodeId) {
  ...
  HLCTimestamp update(HLCTimestamp remoteTimestamp) {
    final now = DateTime.now().millisecondsSinceEpoch;
    final maxPhysical = [now, _physicalTime, remoteTimestamp.physicalTime]
        .reduce((a, b) => a > b ? a : b);

    if (maxPhysical == now &&
        now > _physicalTime &&
        now > remoteTimestamp.physicalTime) {
      _physicalTime = now;
      _logicalTime = 0;
  ...
  HLCTimestamp get current =>
      HLCTimestamp(_physicalTime, _logicalTime, _nodeId);
...
class HLCTimestamp implements Comparable<HLCTimestamp> {
  ...
  /// Get wall time (physical time)
  int get wallTime => physicalTime;

  /// Get logical counter
  int get logical => logicalTime;
  ...
  /// Create HLCTimestamp from DateTime
  static HLCTimestamp fromDateTime(DateTime dateTime, String nodeId) {
    return HLCTimestamp(dateTime.millisecondsSinceEpoch, 0, nodeId);
  }

  /// Convert HLCTimestamp to DateTime
  DateTime toDateTime() {
    return DateTime.fromMillisecondsSinceEpoch(physicalTime);
  }
  ...
  factory HLCTimestamp.fromMap(Map<String, dynamic> map) {
    return HLCTimestamp(
      map['physical_time'] as int,
      map['logical_time'] as int,
      map['node_id'] as String,
    );
  }
  ...
  /// Alias for happensAfter for compatibility
  bool isAfter(HLCTimestamp other) => happensAfter(other);

  /// Alias for happensBefore for compatibility
  bool isBefore(HLCTimestamp other) => happensBefore(other);
...
typedef HybridLogicalTimestamp = HLCTimestamp;
