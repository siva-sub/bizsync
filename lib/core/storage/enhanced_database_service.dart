...
class EnhancedDatabaseService {
  ...
  Future<void> createJournalEntry({
    ...
    await transactionManager.runInTransaction((transaction) async {
      ...
      final accountingTransaction = CRDTAccountingTransaction(
        ...
        version: VectorClock(nodeId),
        transactionNumber:
            CRDTRegister(_generateTransactionNumber(), timestamp),
  ...
  Future<Map<String, dynamic>> generateFinancialStatements(
    ...
    return await _doubleEntryService.generateFinancialStatements(
        asOfDate: asOfDate);
  }
  ...
  Future<Map<String, dynamic>> runHealthCheck() async {
    ...
    try {
      ...
      results['fk_violations_details'] =
          fkViolations.map((v) => v.toJson()).toList();
      ...
      results['business_violations_details'] =
          businessViolations.map((v) => v.toJson()).toList();
  ...
  Future<Map<String, dynamic>> exportBackup({
    ...
    for (final table in businessTables) {
      try {
        ...
        backup['tables'][table] = [];
        backup['metadata']
            ['warnings'] = (backup['metadata']['warnings'] as List? ?? [])
    ...
    if (includeCRDTMetadata) {
      ...
      for (final table in metadataTables) {
        try {
          final data =
              await db.query(table, limit: 1000, orderBy: 'created_at DESC');
          backup['tables'][table] = data;
        } catch (e) {
          backup['tables'][table] = [];
        }
    ...
    if (includeAuditTrail) {
      try {
        final auditData = await db.query('audit_trail',
            limit: 10000, orderBy: 'timestamp DESC');
  ...
  Future<Map<String, dynamic>> importBackup(Map<String, dynamic> backupData,
      {bool overwriteConflicts = false}) async {
    final results = <String, dynamic>{
  ...
  Future<void> _importCRDTRecord(
    ...
    await _auditService.logEvent(
      ...
      recordId: recordId,
      eventType:
          existing.isEmpty ? AuditEventType.create : AuditEventType.update,
