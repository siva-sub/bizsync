...
class TradeDutyCalculator {
  ...
  TradeDutyResult calculateImportDuty({
    ...
    final applicableDuties = _getApplicableDuties(item, importDate, DutyType.importDuty);
    ...
    for (final dutyRate in selectedDuties) {
      final dutyAmount = dutyRate.calculateDuty(item.totalValue, quantity: item.quantity);
      totalDutyAmount += dutyAmount;

      // Check for preferential savings
      final standardDuty = _getStandardDuty(item, dutyRate.dutyType);
      final standardAmount = standardDuty?.calculateDuty(item.totalValue, quantity: item.quantity) ?? dutyAmount;
  ...
  List<TradeDutyResult> calculateBulkImportDuty({
    ...
    return items.map((item) => calculateImportDuty(
      item: item,
  ...
  Map<String, dynamic> generateCustomsDeclaration({
    ...
    final totalValue = results.fold<double>(0, (sum, r) => sum + r.item.totalValue);
    final totalDuty = results.fold<double>(0, (sum, r) => sum + r.totalDutyAmount);
  ...
  List<DutyRate> _getApplicableDuties(TradeItem item, DateTime date, DutyType dutyType) {
    final hsCodeRates = _dutyRates[item.hsCode] ?? [];
    final wildcardRates = _dutyRates.entries
        .where((entry) => entry.key.endsWith('*') && 
               item.hsCode.startsWith(entry.key.substring(0, entry.key.length - 1)))
    ...
    return allRates
        .where((rate) => rate.dutyType == dutyType && rate.isApplicableTo(item, date))
        .toList();
  }

  List<DutyRate> _applyPreferentialTreatment(List<DutyRate> duties, TradeItem item) {
  ...
  DutyRate? _getStandardDuty(TradeItem item, DutyType dutyType) {
    return _dutyRates[item.hsCode]
        ?.where((rate) => rate.dutyType == dutyType && rate.preferentialScheme == null)
        .firstOrNull;
  }
  ...
  Map<String, dynamic> _calculatePreferentialBenefits(List<DutyBreakdown> breakdowns) {
    final schemes = <String, double>{};
    double totalSavings = 0;
    
    for (final breakdown in breakdowns) {
      if (breakdown.preferentialSavings != null && breakdown.preferentialSavings! > 0) {
        final scheme = breakdown.preferentialScheme ?? 'Standard';
        schemes[scheme] = (schemes[scheme] ?? 0) + breakdown.preferentialSavings!;
    ...
    return {
      ...
      'savingsPercentage': totalSavings > 0 ? 
          (totalSavings / breakdowns.fold<double>(0, (sum, b) => sum + (b.standardRate ?? b.rate))) * 100 : 0,
    };
  }

  Map<String, dynamic> _summarizePreferentialBenefits(List<TradeDutyResult> results) {
    ...
    for (final result in results) {
      ...
      if (benefits['schemeBreakdown'] != null) {
        final schemeBreakdown = benefits['schemeBreakdown'] as Map<String, dynamic>;
  ...
  bool verifyCertificateOfOrigin(Map<String, dynamic> certificate) {
    final requiredFields = ['exporterName', 'importerName', 'goodsDescription', 
                           'originCountry', 'issueDate', 'certificateNumber'];
