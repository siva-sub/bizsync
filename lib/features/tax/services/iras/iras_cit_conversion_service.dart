...
class IrasCitConversionService {
    ...
    try {
      ...
      await _auditService.logOperation(
        ...
        details: {
          ...
          'has_assets': {
            'non_hp_equipment': request.nonHPCompCommEquipment?.isNotEmpty ?? false,
            'non_hp_ppe': request.nonHpOtherPPE?.isNotEmpty ?? false,
            'low_value_assets': request.nonHpOtherPPE_LowValueAsset?.isNotEmpty ?? false,
      ...
      if (response.isSuccess) {
        ...
        if (kDebugMode) {
          print('✅ CIT conversion completed successfully (Return Code: ${response.returnCode})');
          print('📊 Generated documents:');
          print('   • Profit & Loss Statement: ${response.data?.dataDtlPNL != null ? "✓" : "✗"}');
          print('   • Tax Computation: ${response.data?.dataTCSch != null ? "✓" : "✗"}');
          print('   • Form C-S: ${response.data?.dataFormCS != null ? "✓" : "✗"}');
          print('   • Capital Allowance Schedule: ${response.data?.dataCASch != null ? "✓" : "✗"}');
          print('   • Medical Expense Schedule: ${response.data?.dataMedExpSch != null ? "✓" : "✗"}');
          print('   • Rental Schedule: ${response.data?.dataRentalSch != null ? "✓" : "✗"}');
          print('   • Renovation & Refurbishment Schedule: ${response.data?.dataRRSch != null ? "✓" : "✗"}');
        }
      } else {
  ...
  Map<String, dynamic>? extractProfitLossStatement(CitConversionResponse response) {
  ...
  Map<String, dynamic>? extractCapitalAllowanceSchedule(CitConversionResponse response) {
    if (!response.isSuccess) return null;
    return response.data?.dataCASch;
  }
  
  /// Extract medical expense schedule from conversion response
  Map<String, dynamic>? extractMedicalExpenseSchedule(CitConversionResponse response) {
    if (!response.isSuccess) return null;
  ...
  Map<String, dynamic>? extractRenovationSchedule(CitConversionResponse response) {
    if (!response.isSuccess) return null;
  ...
  double? extractChargeableIncome(CitConversionResponse response) {
    ...
    if (taxComp != null && taxComp.containsKey('chargeableIncomeAftTaxExemptAmt')) {
      return double.tryParse(taxComp['chargeableIncomeAftTaxExemptAmt'].toString());
    }
    return null;
  }
  ...
  Map<String, bool> getGeneratedDocumentsSummary(CitConversionResponse response) {
    return {
      'profit_loss_statement': response.data?.dataDtlPNL != null,
      'tax_computation': response.data?.dataTCSch != null,
      'form_cs': response.data?.dataFormCS != null,
  ...
  void _validateCitRequest(CitConversionRequest request) {
    ...
    if (ya == null || ya < 2000 || ya > DateTime.now().year + 1) {
      errors['ya'] = ['Invalid year of assessment (must be between 2000 and ${DateTime.now().year + 1})'];
    }
    ...
    if (request.declaration.isQualifiedToUseConvFormCS != 'true' &&
      ...
      errors['isQualifiedToUseConvFormCS'] = ['Must be "true" or "false" (as string)'];
    }
  ...
  void _validateFinancialData(CitData data, Map<String, List<String>> errors) {
    ...
    if (data.firstYAInWhichS14QDeductionClaimed.isNotEmpty) {
      ...
      if (ya == null || ya < 2000 || ya > DateTime.now().year) {
        errors['firstYAInWhichS14QDeductionClaimed'] = ['Invalid year of assessment'];
      }
    }
  }
  
  /// Validate asset data
  void _validateAssets(CitConversionRequest request, Map<String, List<String>> errors) {
    ...
    if (request.hpOtherPPE != null) {
      for (int i = 0; i < request.hpOtherPPE!.length; i++) {
        ...
        if (double.tryParse(asset.depositOrPrincipalExcludingInterestIncludingDownpaymentEachAsset) == null) {
          errors['${asset.runtimeType}[$i].depositOrPrincipal'] = ['Must be a valid number'];
        }
      }
    }
  }
  ...
  void _validateAsset(CitAsset asset, String fieldPrefix, Map<String, List<String>> errors) {
    ...
    if (asset.yaOfDisposalEachAsset != null && asset.yaOfDisposalEachAsset!.isNotEmpty) {
      final disposalYa = int.tryParse(asset.yaOfDisposalEachAsset!);
      if (disposalYa == null || disposalYa < 1900 || disposalYa > DateTime.now().year) {
        errors['$fieldPrefix.yaOfDisposal'] = ['Invalid year of disposal'];
      }
      
      if (asset.salesProceedEachAsset != null && asset.salesProceedEachAsset!.isNotEmpty) {
        if (double.tryParse(asset.salesProceedEachAsset!) == null) {
          errors['$fieldPrefix.salesProceed'] = ['Sales proceed must be a valid number'];
        }
      }
    }
  }
