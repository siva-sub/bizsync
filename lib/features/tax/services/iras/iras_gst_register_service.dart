...
class IrasGstRegisterService {
    ...
    try {
      ...
      if (!_isValidRegistrationId(registrationId)) {
        throw const IrasValidationException(
          ...
          {
            'regID': [
              'Must be a valid GST registration number (MXXXXXXXX), UEN (XXXXXXXXXXX), or NRIC (SXXXXXXXX/TXXXXXXXX)'
            ]
          },
        );
      }
      ...
      if (response.isSuccess) {
        ...
        await _auditService.logFailure(
          ...
          entityId: registrationId,
          error:
              'GST register check failed with return code: ${response.returnCode}',
          details: {
  ...
  Future<bool> isGstRegistered(String registrationId,
      {String? clientId}) async {
    try {
      final response =
          await checkGstRegister(registrationId, clientId: clientId);
  ...
  Future<bool> hasActiveGstRegistration(String registrationId,
      {String? clientId}) async {
    try {
      final response =
          await checkGstRegister(registrationId, clientId: clientId);
  ...
  bool _isValidRegistrationId(String regId) {
    ...
    final uenPattern =
        RegExp(r'^(\d{8}[A-Z]|[A-Z]\d{2}[A-Z]{2}\d{4}[A-Z]|\d{9}[A-Z])$');
