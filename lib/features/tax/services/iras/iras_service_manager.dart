...
class IrasServiceManager {
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      if (!credentialsValid) {
        throw const IrasConfigException('IRAS credentials integrity check failed');
      }
  ...
  Future<Map<String, dynamic>?> getGstRegistrationDetails(String registrationId) async {
    final service = await gstRegister;
  ...
  Future<Map<String, dynamic>> convertToFormCS({
    ...
    return {
      'success': true,
      'message': 'CIT conversion service initialized - implement with actual financial data',
...
extension IrasServiceManagerExtensions on IrasServiceManager {
  ...
  Future<String> startAuth(String callback) => initiateSingPassAuth(callbackUrl: callback);
