...
class IrasEmploymentService {
  ...
  Future<EmploymentIncomeSubmissionResponse> submitEmploymentRecords(
    ...
    try {
      ...
      final response =
          EmploymentIncomeSubmissionResponse.fromJson(responseData);
      ...
      if (response.isSuccess) {
        ...
        if (kDebugMode) {
          ...
          print(
              '📊 Summary: ${response.data?.summary.successfulRecords}/${response.data?.summary.totalRecords} successful');
        }
      } else {
        await _auditService.logFailure(
          ...
          entityId: 'bulk_submission',
          error:
              'Employment submission failed with return code: ${response.returnCode}',
  ...
  void _validateEmploymentRequest(EmploymentIncomeSubmissionRequest request) {
    ...
    if (request.ir8aInput == null &&
      ...
      errors['inputs'] = [
        'At least one input (IR8A, IR8S, A8A, or A8B) must be provided'
      ];
    }
    ...
    if (errors.isNotEmpty) {
      throw IrasValidationException(
          'Employment submission validation failed', errors);
    }
  }
  ...
  String _convertA8aToCsv(List<A8aFormData> records) {
    ...
    const header =
        'UEN,YA,NRIC,Employee Name,Benefit Type,Description,Value,Period';
    final rows = <String>[];
  ...
  String _convertA8bToCsv(List<A8bFormData> records) {
    ...
    const header =
        'UEN,YA,NRIC,Employee Name,Grant Date,Exercise Date,Shares,Exercise Price,Market Price,Benefit Value';
    final rows = <String>[];
