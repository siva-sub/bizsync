    ...
    while (attempts < IrasConfig.maxRetries) {
      try {
        ...
        if (response.statusCode == 200) {
          ...
          IrasConfig.logApiResponse(endpoint, response.statusCode, responseData);
          
          // Check IRAS return code
          if (responseData.containsKey('returnCode')) {
            final returnCode = responseData['returnCode'] as int;
            if (returnCode != 10 && returnCode != 30) { // 10 = success, 30 = success with data
...
class TestableIrasApiClient extends IrasApiClient {
  TestableIrasApiClient(http.Client httpClient) : super._(httpClient: httpClient);
}