...
class IrasAuthService {
  ...
  Duration? get timeUntilExpiry => isAuthenticated 
      ? _tokenExpiry!.difference(DateTime.now())
      : null;
  ...
  Future<String> initiateSingPassAuth({
    ...
    try {
      ...
      throw IrasAuthException('SingPass auth initiation failed: ${e.userFriendlyMessage}');
    }
  }
  ...
  Future<void> completeSingPassAuth({
    ...
    if (_currentState != state) {
      throw const IrasAuthException('Invalid state parameter - possible CSRF attack');
    }
    ...
    try {
      ...
      throw IrasAuthException('SingPass auth completion failed: ${e.userFriendlyMessage}');
    }
  }
  ...
  String _generateState() {
    ...
    final random = List.generate(16, (i) => 
  ...
  static const Map<String, List<String>> commonScopes = {
    ...
    'full': ['GSTReturnsSub', 'GSTTransListSub', 'EmpIncomeRecordsSub', 'CorporateTaxSub'],
  };
}