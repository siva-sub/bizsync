...
class BackupFormatHandler {
  ...
  static Future<String> createBackupFile({
    ...
    try {
      ...
      for (final entry in tableData.entries) {
        ...
        final tableFile =
            File('${tempDir.path}/$dataDirectoryName/$tableName.json');
      ...
      for (final entry in attachments.entries) {
        ...
        final attachmentFile =
            File('${tempDir.path}/$attachmentsDirectoryName/$fileName');
      ...
      final checksumsContent =
          checksums.entries.map((e) => '${e.key}:${e.value}').join('\n');
      ...
      compressedData =
          Uint8List.fromList(zlibEncoder.encode(archiveBytes, level: 6));
    ...
    if (password != null && password.isNotEmpty) {
      try {
        ...
        throw BackupException(
            'Failed to decrypt backup: Invalid password or corrupted file');
      }
    }
  ...
  static Future<bool> _validateIntegrity(
      String extractPath, BackupManifest manifest) async {
    try {
      // Read checksums file
      ...
      for (final table in manifest.tables) {
        final tableFile =
            File('$extractPath/$dataDirectoryName/${table.name}.json');
      ...
      for (final attachment in manifest.attachments) {
        final attachmentFile =
            File('$extractPath/$attachmentsDirectoryName/${attachment.name}');
        ...
        final expectedChecksum =
            expectedChecksums['attachment_${attachment.name}'];
  ...
  static Future<Uint8List> _decryptData(
  ...
  static String _generateBackupFileName(BackupManifest manifest) {
    final timestamp = manifest.createdAt
        .toIso8601String()
        .replaceAll(':', '-')
        .replaceAll('.', '-');
