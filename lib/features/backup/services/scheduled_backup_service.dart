...
class ScheduledBackupService extends ChangeNotifier {
  ...
  Future<void> scheduleBackups({
    ...
    try {
      ...
      debugPrint('Scheduled backups enabled: ${type.name} every ${_formatDuration(interval)}');
    } catch (e) {
  ...
  static Future<void> executeScheduledBackup(Map<String, dynamic> inputData) async {
    try {
      ...
      final encryptionEnabled = inputData['encryption_enabled'] as bool? ?? true;
      
      // Get default backup directory
      final documentsDir = await getApplicationDocumentsDirectory();
      final backupDir = Directory(path.join(documentsDir.path, 'backups', 'scheduled'));
      ...
      final backupFileName = 'auto_backup_${DateTime.now().toIso8601String()}.bdb';
      ...
      debugPrint('Type: ${backupType.name}, Scope: ${backupScope.name}, Encrypted: $encryptionEnabled');
...
final scheduledBackupServiceProvider = ChangeNotifierProvider<ScheduledBackupService>((ref) {
