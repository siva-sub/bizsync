    ...
    try {
      ...
      if (customPath != null) {
        ...
        final String finalFileName = fileName ??
            'payNow_qr_${DateTime.now().millisecondsSinceEpoch}.png';
  ...
  static Future<QRSaveResult> saveQRDataAsText({
    ...
    try {
      ...
      if (customPath != null) {
        ...
        final Directory directory = await getApplicationDocumentsDirectory();
        final String finalFileName = fileName ??
            'payNow_qr_data_${DateTime.now().millisecondsSinceEpoch}.txt';
        savePath = '${directory.path}/$finalFileName';
      }
  ...
  static Future<QRPackageResult> createQRPackage({
    ...
    try {
      ...
      final String finalPackageName =
          packageName ?? 'payNow_qr_package_$timestamp';

      // Create package directory
      final Directory appDir = await getApplicationDocumentsDirectory();
      final Directory packageDir =
          Directory('${appDir.path}/$finalPackageName');
      ...
      for (int i = 0; i < qrDataList.length; i++) {
        try {
          ...
          final String merchantName =
              i < merchantNames.length ? merchantNames[i] : 'Merchant ${i + 1}';
          ...
          final QRPackageResult packageResult = await createQRPackage(
            ...
            branding: branding,
            packageName:
                '${batchDir.path}/qr_${i + 1}_${merchantName.replaceAll(' ', '_')}',
          );
