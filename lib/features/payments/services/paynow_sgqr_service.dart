...
class PayNowSGQRService {
  ...
  static String _buildPayNowData({
    ...
    if (uenNumber != null && uenNumber.isNotEmpty) {
      ...
      final mobile = cleanMobile.startsWith('65') ? cleanMobile.substring(2) : cleanMobile;
  ...
  static String _buildAdditionalDataTemplate({
    ...
    if (reference != null && reference.isNotEmpty) {
      buffer.write(_formatTLV('01', reference.substring(0, min(reference.length, 25))));
    }
    
    // Customer Label (Tag 02) - Optional
    if (description != null && description.isNotEmpty) {
      buffer.write(_formatTLV('02', description.substring(0, min(description.length, 25))));
    }
  ...
  static bool isValidSGQR(String sgqrString) {
    ...
    try {
      ...
      final providedCrc = int.parse(sgqrString.substring(sgqrString.length - 4), radix: 16);
  ...
  static Map<String, dynamic>? parseSGQRMetadata(String sgqrString) {
    ...
    try {
      ...
      if (amountMatch != null) {
        metadata['amount'] = double.tryParse(amountMatch.group(1) ?? '0') ?? 0.0;
      }
      
      // Extract merchant name
      final merchantMatch = RegExp(r'59\d{2}([^6]{1,25})').firstMatch(sgqrString);
