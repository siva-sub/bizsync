...
abstract class ReportService {
  ...
  Future<FinancialReportData> generateFinancialReport(
      DateTime startDate, DateTime endDate);
...
class ReportServiceImpl implements ReportService {
  ...
  Future<SalesReportData> generateSalesReport(
    ...
    final totalRevenue =
        dailySales.fold(0.0, (sum, point) => sum + point.revenue);
    final totalProfit =
        dailySales.fold(0.0, (sum, point) => sum + point.profit);
    final totalTransactions =
        dailySales.fold(0, (sum, point) => sum + point.transactions);
    ...
    return SalesReportData(
      ...
      topCustomers: [
        const CustomerSales(
            customerId: '1',
            customerName: 'Tech Solutions Pte Ltd',
            revenue: 8500,
            transactions: 12),
        const CustomerSales(
            customerId: '2',
            customerName: 'Marketing Agency Pte Ltd',
            revenue: 6200,
            transactions: 8),
        const CustomerSales(
            customerId: '3',
            customerName: 'Startup Pte Ltd',
            revenue: 4800,
            transactions: 6),
      ],
    );
  ...
  @override
  Future<TaxReportData> generateTaxReport(
      DateTime startDate, DateTime endDate) async {
    ...
    final totalGstCollected =
        monthlyGst.fold(0.0, (sum, point) => sum + point.collected);
  ...
  Future<FinancialReportData> generateFinancialReport(
    ...
    const incomeStatement = [
      ...
      IncomeStatementItem(
          category: 'Cost of Goods Sold', amount: -18000, percentage: -40),
      IncomeStatementItem(
          category: 'Gross Profit', amount: 27000, percentage: 60),
      IncomeStatementItem(
          category: 'Operating Expenses', amount: -15000, percentage: -33),
      IncomeStatementItem(
          category: 'Net Income', amount: 12000, percentage: 27),
    ];

    const balanceSheet = [
      BalanceSheetItem(category: 'Cash & Bank', amount: 25000, isAsset: true),
      BalanceSheetItem(
          category: 'Accounts Receivable', amount: 8500, isAsset: true),
      ...
      BalanceSheetItem(
          category: 'Accounts Payable', amount: 6500, isAsset: false),
