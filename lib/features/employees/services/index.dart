...
abstract class IEmployeeService {
  ...
  Future<CRDTEmployee> updateEmployee(String employeeId, Map<String, dynamic> updates);
...
abstract class IPayrollService {
  ...
  Future<CRDTSingaporeCpfCalculation> calculateCpfContributions(Map<String, dynamic> cpfData);
...
abstract class ILeaveManagementService {
  ...
  Future<void> approveLeaveRequest(String leaveRequestId, String approverId, {String? comments});
  Future<void> rejectLeaveRequest(String leaveRequestId, String approverId, {String? comments});
...
abstract class IAttendanceService {
  ...
  Map<String, dynamic> generateAttendanceReport(Map<String, dynamic> reportParams);
}

// Service factory for dependency injection
class EmployeeServiceFactory {
  static EmployeeService createEmployeeService(NotificationService notificationService) {
  ...
  static LeaveManagementService createLeaveManagementService(NotificationService notificationService) {
    return LeaveManagementService(notificationService);
  }
  
  static AttendanceService createAttendanceService(NotificationService notificationService) {
...
class EmployeeManagementFacade {
  ...
  Future<Map<String, dynamic>> getEmployeeDashboard(String employeeId) async {
    ...
    return {
      ...
      'recent_leave_requests': _leaveService.getLeaveRequestsForEmployee(employeeId)
          .take(5)
          .map((request) => request.toJson())
          .toList(),
      'today_attendance': _attendanceService.getAttendanceRecord(employeeId, now)?.toJson(),
    };
  }
  ...
  Future<Map<String, dynamic>> processEndOfMonth({
    ...
    final employees = employeeIds != null
        ? employeeIds.map((id) => _employeeService.getEmployee(id)).whereType<CRDTEmployee>().toList()
