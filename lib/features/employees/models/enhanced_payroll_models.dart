...
class CRDTEnhancedPayrollCalculation implements CRDTModel {
  ...
  void _calculateWageTotals() {
    ...
    final ordinaryWage =
        basicSalaryCents.value + overtimeCents.value + allowancesCents.value;
    if (ordinaryWage > 0) ordinaryWageCents.increment(ordinaryWage);

    // Additional wage = commission + bonus + benefits in kind
    final additionalWage =
        commissionCents.value + bonusCents.value + benefitsInKindCents.value;
  ...
  void _calculateCpfContributions() {
    try {
      final ageCat = CpfAgeCategory.values
          .firstWhere((e) => e.toString().split('.').last == ageCategory.value);
      ...
      final owCeiling =
          CpfContributionRates.ordinaryWageCeiling * 100; // Convert to cents
      final cappedOW = ordinaryWageCents.value > owCeiling
          ? owCeiling
          : ordinaryWageCents.value;
      ...
      if (empContrib + emplerContrib > 0)
        totalCpfCents.increment(empContrib + emplerContrib);
    } catch (e) {
  ...
  void _calculateShgContributions() {
    ...
    final totalShg =
        cdacCents.value + ecfCents.value + mbmfCents.value + sindaCents.value;
    if (totalShg > 0) totalShgCents.increment(totalShg);
  }

  void _calculateTotalDeductions() {
    final totalDed = employeeCpfCents.value +
        totalShgCents.value +
        incomeTaxCents.value +
  ...
  void recordMakeupContribution(
      double amount, DateTime contributionDate, HLCTimestamp timestamp) {
    if (amount > 0) {
