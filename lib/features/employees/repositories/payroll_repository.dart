...
class PayrollRepository {
  ...
  Future<List<CRDTPayrollRecord>> getPayrollRecordsForEmployee(
    ...
    if (fromDate != null) {
      conditions.add(
          'datetime(JSON_EXTRACT(data, "\$.pay_period_start.value") / 1000, \'unixepoch\') >= datetime(?)');
  ...
  Future<void> saveCpfCalculation(
      CRDTSingaporeCpfCalculation cpfCalculation) async {
    await _database.insert(
  ...
  Future<CRDTSingaporeCpfCalculation?> getCpfCalculation(
      String calculationId) async {
  ...
  Future<List<CRDTSingaporeCpfCalculation>> getCpfCalculationsForEmployee(
    ...
    if (fromDate != null) {
      conditions.add(
          'datetime(JSON_EXTRACT(data, "\$.calculation_date.value") / 1000, \'unixepoch\') >= datetime(?)');
      args.add(fromDate.toIso8601String());
    }
    ...
    return results
        .map((data) => CRDTSingaporeCpfCalculation.fromCRDTJson(data))
        .toList();
  }

  /// Update CPF calculation
  Future<void> updateCpfCalculation(
      CRDTSingaporeCpfCalculation cpfCalculation) async {
    await _database.update(
      table: 'cpf_calculations',
      id: cpfCalculation.id,
  ...
  Future<List<CRDTIR8ATaxForm>> getIR8ATaxFormsForYear(int taxYear) async {
    final results = await _database.query(
      table: 'ir8a_tax_forms',
      where:
          'JSON_EXTRACT(data, "\$.tax_year.value") = ? AND JSON_EXTRACT(data, "\$.is_deleted") = ?',
  ...
  Future<List<CRDTPayComponent>> getPayComponentsForPayroll(
      String payrollRecordId) async {
    final results = await _database.query(
      table: 'pay_components',
      where:
          'JSON_EXTRACT(data, "\$.payroll_record_id.value") = ? AND JSON_EXTRACT(data, "\$.is_deleted") = ?',
  ...
  Future<void> batchInsertPayrollRecords(
      List<CRDTPayrollRecord> payrollRecords) async {
  ...
  Future<List<CRDTPayrollRecord>> getPendingPayrollRecords() async {
    final results = await _database.query(
      table: 'payroll_records',
      where:
          'JSON_EXTRACT(data, "\$.status.value") = ? AND JSON_EXTRACT(data, "\$.is_deleted") = ?',
  ...
  Future<List<CRDTPayrollRecord>> getApprovedPayrollRecords({
    ...
    if (payDate != null) {
      conditions.add(
          'date(JSON_EXTRACT(data, "\$.pay_date.value") / 1000, \'unixepoch\') = date(?)');
