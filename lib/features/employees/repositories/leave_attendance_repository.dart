...
class LeaveAttendanceRepository {
  ...
  Future<List<CRDTLeaveRequest>> getLeaveRequestsForEmployee(
    ...
    if (fromDate != null) {
      conditions.add('datetime(JSON_EXTRACT(data, "\$.start_date.value") / 1000, \'unixepoch\') >= datetime(?)');
      args.add(fromDate.toIso8601String());
    }
    
    if (toDate != null) {
      conditions.add('datetime(JSON_EXTRACT(data, "\$.end_date.value") / 1000, \'unixepoch\') <= datetime(?)');
  ...
  Future<List<CRDTLeaveRequest>> getLeaveRequestsInDateRange(
    ...
    if (employeeIds != null && employeeIds.isNotEmpty) {
      ...
      conditions.add('JSON_EXTRACT(data, "\$.employee_id.value") IN ($placeholders)');
  ...
  Future<void> saveAttendanceRecord(CRDTAttendanceRecord attendanceRecord) async {
    await _database.insert(
  ...
  Future<CRDTAttendanceRecord?> getAttendanceRecord(String attendanceRecordId) async {
  ...
  Future<List<CRDTAttendanceRecord>> getAttendanceRecordsForEmployee(
    ...
    return results.map((data) => CRDTAttendanceRecord.fromCRDTJson(data)).toList();
  }
  ...
  Future<void> updateAttendanceRecord(CRDTAttendanceRecord attendanceRecord) async {
    await _database.update(
      table: 'attendance_records',
      id: attendanceRecord.id,
      data: attendanceRecord.toCRDTJson(),
  ...
  Future<void> savePerformanceRecord(CRDTPerformanceRecord performanceRecord) async {
    await _database.insert(
  ...
  Future<List<CRDTPerformanceRecord>> getPerformanceRecordsForEmployee(
    ...
    return results.map((data) => CRDTPerformanceRecord.fromCRDTJson(data)).toList();
  }
  
  /// Update performance record
  Future<void> updatePerformanceRecord(CRDTPerformanceRecord performanceRecord) async {
    await _database.update(
      table: 'performance_records',
      id: performanceRecord.id,
      data: performanceRecord.toCRDTJson(),
  ...
  Future<void> batchInsertAttendanceRecords(List<CRDTAttendanceRecord> records) async {
