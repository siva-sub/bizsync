...
class EmployeeRepository {
  ...
  Future<List<CRDTEmployee>> searchEmployees(
    ...
    final results = await _database.query(
      ...
      whereArgs: args,
      orderBy:
          'JSON_EXTRACT(data, "\$.first_name.value"), JSON_EXTRACT(data, "\$.last_name.value")',
      limit: limit,
    );
  ...
  Future<List<CRDTEmployee>> getEmployeesByManager(String managerId) async {
    final results = await _database.query(
      table: 'employees',
      where:
          'JSON_EXTRACT(data, "\$.manager_id.value") = ? AND JSON_EXTRACT(data, "\$.is_deleted") = ?',
  ...
  Future<List<CRDTEmployee>> getEmployeesByDepartment(String department) async {
    final results = await _database.query(
      table: 'employees',
      where:
          'JSON_EXTRACT(data, "\$.department.value") = ? AND JSON_EXTRACT(data, "\$.is_deleted") = ?',
  ...
  Future<List<CRDTEmployee>> getEmployeesWithExpiringWorkPermits(
      {int daysAhead = 90}) async {
  ...
  Future<Map<String, dynamic>> getEmployeeStatistics() async {
    ...
    stats['departments'] = Map.fromEntries(
      deptResult
          .map((row) => MapEntry(row['department'] as String, row['count'])),
    );
    ...
    stats['employment_types'] = Map.fromEntries(
      typeResult.map(
          (row) => MapEntry(row['employment_type'] as String, row['count'])),
    );
  ...
  Future<List<CRDTEmployee>> getEmployeesModifiedAfter(
  ...
  Future<List<CRDTEmployee>> getEmployeesBySkills(List<String> skills) async {
    ...
    final skillConditions = skills
        .map((skill) => 'JSON_EXTRACT(data, "\$.skills") LIKE ?')
        .join(' OR ');
