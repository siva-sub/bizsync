...
class _SyncProgressScreenState extends State<SyncProgressScreen>
  ...
  void initState() {
    ...
    _sessionSubscription =
        widget.syncService.syncSessionUpdates.listen(_onSessionUpdate);
    _progressSubscription =
        widget.syncService.syncProgressUpdates.listen(_onProgressUpdate);
  ...
  Future<void> _cancelSync() async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content:
            const Text('Are you sure you want to cancel the sync operation?'),
        actions: [
          TextButton(
                    ...
                    children: [
                      ...
                      ..._conflicts.take(3).map((conflict) {
                        return ListTile(
                          ...
                          title:
                              Text('${conflict.itemType}: ${conflict.itemId}'),
                      ...
                      if (_conflicts.length > 3)
                        TextButton(
                          ...
                          child:
                              Text('View all ${_conflicts.length} conflicts'),
                        ),
                    ],
                  ),
                ),
  ...
  Widget _buildProgressDetailsContent(SyncProgress progress) {
    return Column(
      children: [
        ...
        const Divider(),
        _buildDetailRow(
            'Bytes Transferred', _formatBytes(progress.bytesTransferred)),
